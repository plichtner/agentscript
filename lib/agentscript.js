(function() {
  ...
  Util = util = u = {
    ...
    colorStr: function(c) {
      ...
      return (_ref = c.css) != null ? _ref : Color.arrayToColor(c, "css");
    },
    ...
    fillCtx: function(ctx, color) {
      ...
      if (ctx.fillStyle != null) {
        this.setIdentity(ctx);
        ctx.fillStyle = (_ref = color.css) != null ? _ref : Color.convertColor(color, "css");
  ...
  ColorMaps = {
    gradientImageData: function(nColors, stops, locs) {
      var c, ctx, grad, i, _i, _ref;
      stops = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = stops.length; _i < _len; _i++) {
          c = stops[_i];
          _results.push(Color.convertColor(c, "css"));
        }
        return _results;
      })();
      if (locs == null) {
    ...
    basicColors: ["gray", "red", "orange", "brown", "yellow", "green", "lime", "turquoise", "cyan", "skyblue", "blue", "violet", "magenta", "pink"],
    rampStops: function(color, whiteToo) {
      if (whiteToo) {
    ...
    rampColorMap: function(color, width, whiteToo) {
      ...
      return this.gradientColorMap(width, this.rampStops(color, whiteToo));
    },
    netLogoColorMap: function(width, whiteToo) {
      var color, k, map, maps, name, v, _ref;
      if (width == null) {
        width = 10;
      }
      if (whiteToo == null) {
      ...
      map = [];
      maps = {};
      _ref = this.netLogoColors;
      for (name in _ref) {
        color = _ref[name];
        maps[name] = this.rampColorMap(color, width, whiteToo);
    ...
    createSharedMaps: function(whiteToo) {
      ...
      this.Maps.Gray = this.grayColorMap();
      this.Maps.Rgb256 = this.rgbColorMap(8, 8, 4);
      this.Maps.Rgb = this.rgbColorCube(16);
      this.Maps.Safe = this.rgbColorCube(6);
      this.Maps.Jet = this.gradientColorMap(256, this.jetColors);
      this.Maps.NetLogo = this.netLogoColorMap(10);
      return this.Maps.Ramps = this.netLogoColorMap(18, false);
    },
    randomGray: function(min, max) {
      return this.Maps.Gray.randomColor(min, max);
    },
    randomColor: function() {
      return this.Maps.Rgb256.randomColor();
    }
  };
  ...
  shapes = Shapes = (function() {
    ...
    return {
      ...
      shapeToSprite: function(name, color, size, strokeColor) {
        ...
        shape = this[name];
        index = shape.img != null ? name : "" + name + "-" + color.css;
  ...
  Patch = (function() {
    ...
    Patch.prototype.scaleColor = function(c, s) {
      ...
      return this.color = Color.rgbLerp(c, s);
    };
  ...
  Patches = (function(_super) {
    ...
    Patches.prototype.diffuse = function(v, rate, cMap) {
      var dv, dv8, maxVal, minVal, n, nn, p, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref;
      ...
      for (_l = 0, _len3 = this.length; _l < _len3; _l++) {
        ...
        p._diffuseNext = 0;
        if (cMap) {
          p.color = cMap.scaleColor(p[v], minVal, maxVal);
        }
      }
      return null;
    };
